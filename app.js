...
async function runMigrationsAndCleanup() {
    ...
    // ===== ONE-TIME FIX: Missing tasks for May 1st =====
    if (!appData.stats.fixMay1_v1) {
        appData.stats.fixMay1_v1 = true;
        needsSave = true;
        ...
        const fixTimestamp = '2026-05-02T03:00:00.000Z'; // May 1 10PM CDT
        ...
        tasksToFix.forEach(task => {
            ...
            const has = appData.completedHistory.some(h => {
                ...
                return getDateString(d) === '2026-05-01';
            });
            if (!has) {
                appData.completedHistory.unshift({
                    id: 'recurring_' + task.id + '_fix_may1',
