...
async function runMigrationsAndCleanup() {
    ...
    // ===== ONE-TIME FIX: Missing tasks for April 6th =====
    if (!appData.stats.fixApril6_v1) {
        appData.stats.fixApril6_v1 = true;
        needsSave = true;
        ...
        const fixTimestamp = '2026-04-07T03:00:00.000Z'; // April 6 10PM CDT
        ...
        tasksToFix.forEach(task => {
            ...
            const has = appData.completedHistory.some(h => {
                ...
                return getDateString(d) === '2026-04-06';
            });
            if (!has) {
                appData.completedHistory.unshift({
                    id: 'recurring_' + task.id + '_fix_apr6',
